...
visualization of several types of phylogentic networks using the \CRANpkg{ggplot2}
...
Reworded:\\
Splits graphs allow for the definition of con-compatible splits and are most oftin used to visualize Consensus Networks \cite{Holland2004} or Neighbor-Nets \cite{Bryant2004}. This can be either done using the consensusNet or neighbor-net function in \CRANpkg{phangorn}\cite{Schliep2011} or by importing nexus files from SplitsTree \cite{Huson2006}.

We first load the necessary libraries and read in a splits graph
<<>>=
library(phangorn)
library(ggnetworx)
